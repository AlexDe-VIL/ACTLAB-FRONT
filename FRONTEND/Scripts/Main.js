...
// — Redirección al hacer clic en "Ingresar" desde index.html
...
if (location.pathname.endsWith('index.html') || location.pathname === '/') {
  ...
  document.getElementById('toggle-link').onclick = e => {
    ...
    document.getElementById('submit-btn').innerText = isLogin ? 'Ingresar' : 'Registrar';
    document.getElementById('toggle-link').innerText = isLogin ? 'Registrarse' : 'Volver a Iniciar Sesión';
  };
  ...
  document.getElementById('submit-btn').onclick = async () => {
    ...
    const name = isLogin ? undefined : document.getElementById('name').value; // for registration
    const data = await request(path, 'POST', { email, password, name });
    ...
    if (data.token) {
      ...
      alert(data.msg);  // Show error message if login/register fails
    }
  };
}
...
if (token = localStorage.getItem('token')) {
  ...
  // Initialize the user page (user.html)
  if (role === 'user' && location.pathname.endsWith('User.html')) {
    initUser();
  }

  // Initialize the admin page (admin.html)
